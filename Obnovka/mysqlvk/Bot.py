        ...
        for event in longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                if message_text.startswith('/привязать'):
                    ...
                    help_type = parts[1] if len(parts) > 1 else "базовая"

                    # Базовая помощь
                    if help_type == "базовая":
                        help_message = (
                        "📰 | Доступные команды:\n"
                        ...
                        "📩 | /rcon - отправить Rcon команду на сервер.\n» 1 - выбор нужного сервера.\n» 2 - покажет доступные сервера."
                    ...
                    # Помощь для админов
                    elif help_type == "админ":
                        selected_account = get_selected_account(user_id)
                        if not selected_account:
                            send_vk_message(vk_session, user_id, "🚫 | Вы не выбрали аккаунт. Используйте /акк 2 [номер акка].")
                            continue

                        cursorBot.execute("SELECT rank FROM vk_rcon WHERE nickname = %s", (selected_account,))
                        row = cursorBot.fetchone()
                        allowed_ranks = get_bot_allowed_ranks("admin_help")

                        if row and row[0] in allowed_ranks:
                            admin_help_message = (
                                "🖥 | Админские команды:\n"
                                "⛓️ | /report - заблокировать доступ к консоли и ограничить его донатерские возможности.\n"
                                "(от 2lvl)\n"
                                "📦 |/user-list [номер страницы] - список аккаунтов с доступом.\n"
                                "(от 1lvl)\n"
                            ...
                            send_vk_message(vk_session, user_id, "🚫 | У вас нет прав на использование этой команды!")

                    # Помощь для руководства
                    elif help_type == "руководство":
                        if user_id in config['admins']:
                            management_help = (
                                "👑 | Команды руководства:\n"
                                "👑 | /gp [ранг] - Выдать/изменить текущий ранг игрока\n"
                                "📤 | /рассылка [текст] - разослать всем привязанным аккаунтам сообщение о чем-либо.\n"
                                "✨ | /промо создать|список|удалить - управление промокодами в боте.\n» создать [доступ/привилегия] [ранг/прива] [количество] [описание]\n"
                                "🖥 | RCON команды:\n"
                                "🔑 | /givevk [привелегия] [вк уровень] - выдать привязанному игроку без доступа этот доступ (для авто-доната).\n"
                                "🔗 | /set-vk [никнейм] [вк айди] - принудительно привязать акк к ВК.\n"
                                "➖ | /unset-vk [никнейм] - принудительно отвязать акк от ВК."
                            )
                            send_vk_message(vk_session, user_id, management_help)
                        else:
                            send_vk_message(vk_session, user_id, "🚫 | У вас нет прав на использование этой команды!")

                    # Неизвестный тип помощи
                    else:
                        send_vk_message(vk_session, user_id, "помощь")
