            ...
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                if message_text.startswith('/привязать'):
                    ...
                    if selected_account:
                        ...
                        send_vk_reply_message(user_id, message_id, "🚫 | Вы не выбрали аккаунт для отвязки. Используйте /акк выбрать.")
                        ...
                        # Если это RCON команда
                            ...
                            try:
                                ...
                                if row and row[0]:
                                    ...
                                    send_vk_reply_message(user_id, message_id, "🚫 | Вы не выбрали аккаунт для выполнения ркон команды. Используйте /акк выбрать.")
                                    continue
                ...
                if message_text.startswith('/акк'):
                    try:
                        ...
                        send_vk_reply_message(user_id, message_id, "🚫 не выбраны аргументы. Введите /акк [список, выбрать, профиль, принять, отклонить, восстановить, разбан]")
                        continue
                    ...
                    if args == "список":
                        ...
                        try:
                            ...
                            send_vk_reply_message(user_id, message_id, "🚫 | для выбора аккаунта используйте - /акк выбрать [номер аккаунта]")
                            continue
                        ...
                        if selected_account:
                            ...
                            send_vk_reply_message(user_id, message_id, "🚫 | Вы не выбрали аккаунт для просмотра профиля. Используйте /акк выбрать.")
                        ...
                        if selected_account:
                            ...
                            send_vk_reply_message(user_id, message_id, "🚫 | Вы не выбрали аккаунт для принятия запроса. Используйте /акк выбрать.")
                        ...
                        if selected_account:
                            ...
                            send_vk_reply_message(user_id, message_id, "🚫 | Вы не выбрали аккаунт для отклонения запроса. Используйте /акк выбрать.")
                        ...
                        if selected_account:
                            try:
                                ...
                                send_vk_reply_message(user_id, message_id, "🚫 | Неверный формат. Используйте /акк восстановить [новый_пароль].")
                                continue
                            ...
                            send_vk_reply_message(user_id, message_id, "🚫 | Вы не выбрали аккаунт для восстановления пароля. Используйте /акк выбрать.")
                        ...
                        if selected_account:
                            ...
                            send_vk_reply_message(user_id, message_id, "🚫 | Вы не выбрали аккаунт для разблокировки. Используйте /акк выбрать.")
                ...
                if message_text.startswith('/vk-info'):
                    ...
                    if selected_account is None:
                        send_vk_reply_message(user_id, message_id, "🚫 | Вы не выбрали аккаунт. Используйте /акк выбрать.")
                        continue
                ...
                if message_text.startswith('/промо'):
                    ...
                    if len(args) >= 2 and args[1] == 'список':
                        if not is_admin:
                            send_vk_reply_message(user_id, message_id, "🚫 | У вас нет прав для просмотра списка промокодов.")
                            continue
                        ...
                        if not promos:
                            send_vk_reply_message(user_id, message_id, "📦 | Нет доступных промокодов на данной странице.")
                        else:
                            message = "📦 | Все промо\n\n"
                            for promo in promos:
                    ...
                    # /промо создать — доступ только для администраторов
                        ...
                        if not is_admin:
                            send_vk_reply_message(user_id, message_id, "🚫 | У вас нет прав для создания промокодов.")
                            continue

                        # Проверка, что введены все параметры
                        if len(args) < 6:
                            send_vk_reply_message(user_id, message_id, "🚫 | Неверный формат команды. Используйте: /промо создать [доступ/привилегия] [ранг/привилегия] [количество использований] [описание]")
                            continue
                        ...
                        if not promo_count.isdigit():
                            send_vk_reply_message(user_id, message_id, "🚫 | Количество использований должно быть числом.")
                            continue
                        ...
                        if promo_type == 'доступ':
                            ...
                            send_vk_reply_message(user_id, message_id, "🚫 | Неверная категория промокода. Доступны только 'доступ' и 'привилегия' и 'деньги' .")
                            continue

                        promo_conn.commit()
                        send_vk_reply_message(user_id, message_id, f"✅ Промокод '{promo_name}' успешно создан!")
                        continue
                    ...
                    # /промо активация
                        ...
                        if not promo:
                            send_vk_reply_message(user_id, message_id, "🚫 | Промокод не найден или введен неверно.")
                            continue
                        ...
                        if not selected_account:
                            send_vk_reply_message(user_id, message_id, "🚫 | Привязанный аккаунт не найден.")
                            continue
                        ...
                        if promo_cursor.fetchone():
                            send_vk_reply_message(user_id, message_id, "🚫 | Вы уже активировали этот промокод.")
                            continue

                        # Проверка оставшихся активаций
                        if promo_count <= 0:
                            send_vk_reply_message(user_id, message_id, "🚫 | Этот промокод больше не может быть использован.")
                            continue
                        ...
                        if promo_type == 'доступ':
                            ...
                            if user_rank < promo_rank:
                                ...
                                conn.commit()
                                send_vk_reply_message(user_id, message_id, f"📦 | Промокод активирован на акке {selected_account}!\n🌟 | Описание промо: {promo_desc.replace('_', ' ')}\n👑 | Доступ получен: {promo_rank}")
                            else:
                                send_vk_reply_message(user_id, message_id, "🚫 | Вы не можете использовать этот промокод. Ваш ранг выше выдаваемого.")

                        elif promo_type == 'привилегия':
                            if not server_name:
                                send_vk_reply_message(user_id, message_id, "🚫 | Укажите сервер для активации привилегии в конце.")
                                continue

                            if server_name not in config['servers']:
                                send_vk_reply_message(user_id, message_id, f"🚫 | Сервер '{server_name}' не найден.")
                                continue
                            ...
                            if rcon_response:
                                ...
                                send_vk_reply_message(user_id, message_id, "🚫 | Ошибка при выдаче привилегии на сервере.\n{rcon_response.replace('[PurePerms ]', '👑 | ')}")

                        elif promo_type == 'деньги':
                            if not server_name:
                                send_vk_reply_message(user_id, message_id, "🚫 | Укажите сервер для активации денег в конце.")
                                continue
                            ...
                            if rcon_response:
                                ...
                                send_vk_reply_message(user_id, message_id, "🚫 | Ошибка при выдаче денег на сервере.\n{rcon_response.replace('(Экономика) ', '💵 | ')}")
                        ...
                        send_vk_reply_message(user_id, message_id, "🚫 | Неверная команда Используйте /промо список|активация|создать.")
  #              else:
