...
def send_vk_message(user_id, message_id, message):
    vk.messages.send(
        user_id=user_id,
        message=message,
            ...
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                if message_text == 'помощь админ':
                    ...
                    if rank not in valid_ranks:
                        send_vk_reply_message(user_id, message_id, f"🚫 | Неверный ранг. Доступные ранги: {', '.join(valid_ranks)}.")
                        continue
                    ...
                    if vk_id_row:
                        ...
                        send_vk_reply_message(user_id, message_id, "🚫 | Ник не привязан или привязка не завершена.")
                    ...
                    try:
                        ...
                        if banned_status and banned_status[0] == 'YES':
                            send_vk_reply_message(user_id, message_id, f"🚫 | {selected_account}, вы не можете использовать эту команду, так как заблокированы в консоли.")
                            continue
                        ...
                        if data.startswith('-'):
                            ...
                            if not username.islower():
                                send_vk_reply_message(user_id, message_id, "🚫 | Никнейм игрока должен быть в нижнем регистре.")
                                continue
                            ...
                            if row:
                                ...
                                if banned_status == "NO":
                                    send_vk_reply_message(user_id, message_id, "🚫 | Игрок не находится в бане.")
                                    continue

                                unban_player(username, user_id)
                                send_vk_reply_message(user_id, message_id, f"✅ | Игрок {username} был разбанен в консоли сервера.")
                            else:
                                send_vk_reply_message(user_id, message_id, "🚫 | Игрок не найден или не привязан к консоли.")
                        else:
                            # Бан игрока
                            ...
                            if row:
                                ...
                                send_vk_reply_message(user_id, message_id, f"🚫 | Игрок {username} был заблокирован в консоли сервера.")
                            else:
                                send_vk_reply_message(user_id, message_id, "🚫 | Игрок не найден или не привязан к консоли.")

                    except ValueError:
                        send_vk_reply_message(user_id, message_id, "🚫 | Неверный формат. Используйте /report [ник игрока] [причина] [время в часах].\nили /report [-ник_игрока] для разбана.")
