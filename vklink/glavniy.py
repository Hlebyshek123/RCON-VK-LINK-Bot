...
auth_db_path = '/root/VDS/grif/sv/plugins/VKAuth/auth.db'
playtime_db_path = '/root/VDS/grif/sv/plugins/vkplaytime/playtime.db'
vk_bans_id_path = '/root/VDS/grif/sv/plugins/HlebBans_src/vk_id.db'
vk_protect_path = '/root/VDS/grif/sv/plugins/vkProtection/data/store.db'
...
def execute_rcon_command(user_id, command, *args):
    try:
        ...
        if response.strip():  # Проверяем, что ответ не пустой или не состоит из одних пробелов
            send_vk_reply_message(user_id, f"✅ | Вы успешно отправили команду на сервер {selected_server} (by {selected_account}) :\n {response}")
        else:
            # Сообщение, если сервер вернул пустой ответ
            send_vk_reply_message(user_id, f"✅ | Вы успешно отправили команду на сервер {selected_server} (by {selected_account}) :\n📩 | сервер вернул пустой ответ")
    
    except Exception as e:
        send_vk_reply_message(user_id, f"🚫 | Ошибка при выполнении команды ({selected_server}): \n{str(e)}")
            ...
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                if message_text.startswith('/привязать'):
                    ...
                    if user_id in last_unlink_time:
                        ...
                        if elapsed_time < 1800:  # 1800 секунд = 30 минут
                            ...
                            send_vk_message(user_id, f"🚫 | Чтоб снова привязать аккаунт нужно подождать {remaining_time} минут.")
                            continue
                    ...
                    try:
                        ...
                        send_vk_message(user_id, "🚫 | Неверный формат. Используйте /привязать [ник] [код].")
                        continue
                    ...
                    if account_count >= max_accounts:
                        send_vk_message(user_id, f"🚫 | Вы достигли лимита на привязку аккаунтов ({max_accounts} акков).")
                        continue
                    ...
                    row = cursor.fetchone()
                    if row:
                        send_vk_message(user_id, f"🚫 | Вы уже привязали аккаунт {username} к этому профилю.")
                        continue
                    ...
                    row = cursor.fetchone()
                    if row:
                        send_vk_message(user_id, f"🚫 | Этот аккаунт {username} уже привязан к другому профилю!")
                        continue
                            ...
                            for confirmation_event in longpoll.listen():
                                if confirmation_event.type == VkEventType.MESSAGE_NEW and confirmation_event.to_me and confirmation_event.user_id == user_id:
                                    if confirmation_event.text.strip() == 'ПОДТВЕРЖДАЮ':
                                        ...
                                        conn_bans.commit()
                                        send_vk_message(user_id, f"😨 | Аккаунт {selected_account} успешно отвязан от вк.")
                                        # Сохранение времени отвязки
                                        last_unlink_time[user_id] = time.time()
                                        confirmation_received = True
                                        break
                                    else:
                                        send_vk_message(user_id, "✅ | Вы отменили процесс отвязки.")
                                        confirmation_received = True
                                        break
                        else:
                            send_vk_message(user_id, f"🚫 | Аккаунт {selected_account} не найден.")
                    else:
                        send_vk_message(user_id, "🚫 | Вы не выбрали аккаунт для отвязки. Используйте /аккаунт выбрать.")
                ...
                if message_text == 'помощь админ':
                    ...
                    if row:
                        ...
                        if user_rank in allowed_ranks:
                            ...
                            send_vk_message(user_id, admin_help_message)
                        else:
                            # Если ранг не соответствует, выводим сообщение о недостатке прав
                            send_vk_message(user_id, "🚫 | У вас недостаточно прав для просмотра админ команд.")
                    else:
                        # Если данных о пользователе нет в таблице vk_rcon
                        send_vk_message(user_id, "🚫 | У вас нет прав для просмотра админ команд.")
                    ...
                    if vk_id_row:
                        vk_id = vk_id_row[0]
                        cursor.execute("REPLACE INTO vk_rcon (nickname, vk_id, rank, selected_server) VALUES (?, ?, ?, ?)", (username, vk_id, rank, selected_server))
                        conn.commit()
                        send_vk_message(user_id, f"✅ | Ранг {rank} успешно установлен для [id{vk_id}|{username}].")
                    else:
                    ...
                    try:
                        ...
                        if command_parts[1] == "выбрать":
                            ...
                            if not selected_account:
                                send_vk_reply_message(user_id, "🚫 | У вас не выбран аккаунт для выполнения команды.")
                                continue
                            ...
                            if server_name not in allowed_servers:
                                send_vk_reply_message(user_id, f"🚫 | Сервер {server_name} недоступен для вашему рангy.")
                            else:
                                # Сохраняем выбранный сервер
                                select_server(server_name, user_id)
                                send_vk_reply_message(user_id, f"🖥 Вы успешно выбрали сервер {server_name} с аккаунта {selected_account}")
                        ...
                        # Подкоманда /rcon сервера
                            ...
                            if allowed_servers:
                                ...
                                for i, server in enumerate(allowed_servers, start=1):
                                    message += f"✨ {i}. {server}\n"
                                send_vk_reply_message(user_id, message)
                            else:
                                send_vk_reply_message(user_id, f"🚫 | {selected_account}, у вас нет доступа к серверам.")
                        ...
                        # Если это RCON команда
                            ...
                            try:
                                ...
                                if row and row[0]:
                                    ...
                                    send_vk_reply_message(user_id, "🚫 | Вы не выбрали аккаунт для выполнения ркон команды. Используйте /аккаунт выбрать.")
                                    continue
                                ...
                                if rcon_row:
                                    ...
                                    if banned == 'YES':
                                        ...
                                        if remaining_time.total_seconds() > 0:
                                            ...
                                            send_vk_reply_message(user_id, f"🚫 | [id{user_id}|{selected_account}], вы были заблокированы в консоли сервера. по причине: {ban_reason}. разбан через: {remaining_time_formatted}.")
                                            continue
                                        else:
                                    ...
                                    if '*' in allowed_commands or command in allowed_commands:
                                        if command == "fban":
                                            if len(arguments) >= 3:
                                                ...
                                                execute_rcon_command(user_id, "fban", target_nickname, duration, ban_reason_with_account)
                                                #send_vk_message(user_id, f"⛓️ | Игрок {target_nickname} был забанен на {duration} минут по причине: {ban_reason_with_account}.")
                                            else:
                                                send_vk_reply_message(user_id, "🚫 | Неверный формат. Используйте: /rcon fban [ник] [время] [причина].")
                                        elif command == "say":
                                            # Создаем сообщение с префиксом и именем пользователя
                                            message = ' '.join(arguments)
                                            formatted_message = f"{message} (by {nickname})"
                                            execute_rcon_command(user_id, "say", formatted_message)
                                        elif command == "fkick":
                                            if len(arguments) >= 2:
                                                target_nickname = arguments[0]
                                                ...
                                                execute_rcon_command(user_id, "fkick", target_nickname, kick_reason_with_account)
                                                #send_vk_message(user_id, f"⛓️ | Игрок {target_nickname} был кикнут с сервера по причине: {kick_reason_with_account}.")
                                            else:
                                                send_vk_reply_message(user_id, "🚫 | Неверный формат. Используйте: /rcon fkick [ник] [причина].")
                                        elif command == "fmute":
                                            if len(arguments) >= 3:
                                                target_nickname = arguments[0]
                                                duration = arguments[1]
                                                ...
                                                execute_rcon_command(user_id, "fmute", target_nickname, duration, mute_reason_with_account)
                                                #send_vk_message(user_id, f"🔇 | Игрок {target_nickname} был замучен на {duration} минут по причине: {mute_reason_with_account}.")
                                            else:
                                                send_vk_reply_message(user_id, "🚫 | Неверный формат. Используйте: /rcon fmute [ник] [время] [причина].")
                                        else:
                                            # Выполнение произвольной команды RCON
                                            execute_rcon_command(user_id, command, *arguments)
                                    else:
                                        send_vk_reply_message(user_id, "🚫 | У вас нет прав для выполнения этой команды.")
                                else:
                                    send_vk_reply_message(user_id, "🚫 | Ваш ркон ранг не установлен или вы не привязаны.")
                            except ValueError:
                                send_vk_reply_message(user_id, "🚫 | Неверный формат. Используйте /rcon [команда] [аргументы].")

                    except ValueError:
                        send_vk_reply_message(user_id, "🚫 | Неверный формат команды.")
                    ...
                    if user_id in config['admins']:
                        ...
                        if reply_message_text:
                            ...
                            if users:
                                ...
                                send_vk_message(user_id, "✅ | Сообщение успешно отправлено всем пользователям с активированной рассылкой.")
                            else:
                                send_vk_message(user_id, "🚫 | Нет пользователей с активированной рассылкой.")
                        else:
                            send_vk_message(user_id, "🚫 | Вы не указали сообщение для рассылки.")
                    else:
                        send_vk_message(user_id, "🚫 | У вас нет прав для выполнения команды.")
                    ...
                    try:
                        ...
                        if data.startswith('-'):
                            ...
                            if row:
                                ...
                                send_vk_message(user_id, "🚫 | Игрок не найден или не привязан к консоли.")
                        else:
                            # Бан игрока
                ...
                if message_text.startswith('/аккаунт'):
                    try:
                        ...
                        send_vk_message(user_id, "🚫 не выбраны аргументы. Введите /аккаунт [список, выбрать, профиль, принять, отклонить, восстановить, разбан]")
                        continue
                    ...
                    if args == "список":
                        ...
                        if accounts:
                            ...
                            send_vk_message(user_id, f"🕹 | Ваши привязанные аккаунты:\n{account_list}\n📰 | помощь - помощь")
                        else:
                            send_vk_message(user_id, "🚫 | У вас нет привязанных аккаунтов.")
                        ...
                        try:
                            ...
                            send_vk_message(user_id, "🚫 | для выбора аккаунта используйте - /аккаунт выбрать [номер аккаунта]")
                            continue
                        ...
                        if 0 <= account_number < len(accounts):
                            ...
                            conn.commit()
                            send_vk_message(user_id, f"✅ | Аккаунт '{selected_account}' успешно выбран для дальнейших действий!")
                        else:
                            send_vk_message(user_id, "🚫 | Некорректный номер аккаунта.")
                        ...
                        if selected_account:
                            ...
                            if row:
                                ...
                                send_vk_message(user_id, f"📜 | Информация по аккаунту {username}: \n"
                                                          f"🔰 | ВК ID: {vk_id}\n"
                                                          f"👑 | Доступ: {rank}\n"
                                ...
                                send_vk_message(user_id, "🚫 | Ваш аккаунт не привязан или привязка не завершена.")
                        else:
                            send_vk_message(user_id, "🚫 | Вы не выбрали аккаунт для просмотра профиля. Используйте /аккаунт выбрать.")
                        ...
                        if selected_account:
                            ...
                            if row:
                                ...
                                conn.commit()
                                send_vk_message(user_id, f"✅ | Вход успешно одобрен для аккаунта '{selected_account}'.")
                            else:
                                send_vk_message(user_id, "🚫 | У вас нет активных запросов или вы не в игре.")
                        else:
                            send_vk_message(user_id, "🚫 | Вы не выбрали аккаунт для принятия запроса. Используйте /аккаунт выбрать.")
                        ...
                        if selected_account:
                            ...
                            if row:
                                ...
                                conn.commit()
                                send_vk_message(user_id, f"⛔ | Вход успешно отклонён для аккаунта '{selected_account}'.")
                            else:
                                send_vk_message(user_id, "🚫 | У вас нет активных запросов или вы не в игре.")
                        else:
                            send_vk_message(user_id, "🚫 | Вы не выбрали аккаунт для отклонения запроса. Используйте /аккаунт выбрать.")
                        ...
                        if selected_account:
                            try:
                                ...
                                send_vk_message(user_id, "🚫 | Неверный формат. Используйте /аккаунт восстановить [новый_пароль].")
                                continue
                            ...
                            if row:
                                ...
                                if last_reset_row is None or last_reset_row[0] is None:
                                    ...
                                    if time_diff < timedelta(hours=1):
                                        ...
                                        send_vk_message(user_id, f"🚫 | Вы уже использовали команду. Подождите {hours} ч. {minutes} м. для повторного использования.")
                                    ...
                                    if confirmation_event.type == VkEventType.MESSAGE_NEW and confirmation_event.to_me and confirmation_event.user_id == user_id:
                                        if confirmation_event.text.strip() == 'ПОДТВЕРЖДАЮ':
                                            ...
                                            try:
                                                ...
                                                auth_conn.commit()
                                                send_vk_message(user_id, f"✅ | Вы успешно сменили пароль для аккаунта '{username}'")
                                                ...
                                                send_vk_message(user_id, "🚫 | Ошибка при обновлении пароля. Попробуйте позже.")
                                            finally:
                                                auth_conn.close()
                                            confirmation_received = True
                                            break
                                        else:
                                            send_vk_message(user_id, "✄1�7 | Вы отменили процесс смены пароля.")
                                            confirmation_received = True
                                            break
                                
                                if not confirmation_received:
                                    send_vk_message(user_id, "🚫 | Процесс смены пароля был прерван.")
                            else:
                                send_vk_message(user_id, "🚫 | Аккаунт не найден или не привязан.")
                        else:
                            send_vk_message(user_id, "🚫 | Вы не выбрали аккаунт для восстановления пароля. Используйте /аккаунт выбрать.")
                        ...
                        if selected_account:
                            ...
                            if row:
                                ...
                                cursor.execute("UPDATE vk_links SET process = 'approved' WHERE username = ?", (selected_account,))
                                conn.commit()
                                send_vk_message(user_id, f"✅ | Аккаунт '{selected_account}' успешно разблокирован!")
                            else:
                                # Если аккаунт не заблокирован
                                send_vk_message(user_id, "🚫 | Аккаунт не заблокирован системой 2FA.")
                        else:
                            send_vk_message(user_id, "🚫 | Вы не выбрали аккаунт для разблокировки. Используйте /аккаунт выбрать.")
                ...
                if message_text.startswith('/vk-info'):
                    ...
                    if not user_data:
                        send_vk_message(user_id, "🚫 | Никнейм не привязан ВК.\n (попробуй в нижнем регистре ник)")
                        continue
                    ...
                    send_vk_message(user_id, player_info_message)
                    ...
                    if selected_account is None:
                        send_vk_message(user_id, "🚫 | Вы не выбрали аккаунт. Используйте /аккаунт выбрать.")
                        continue
                    ...
                    if len(args) == 3:
                        ...
                        if value == 'вкл':
                            ...
                            send_vk_message(user_id, "🚫 | Неверное значение. Используйте 'вкл' или 'выкл'.")
                            continue

                        if option == 'рассылка':
                            cursor.execute("UPDATE settings SET mailing = ? WHERE nickname = ?", (new_value, selected_account))
                            conn.commit()
                            send_vk_message(user_id, f"⚙️ | Опция 'рассылка' для аккаунта {selected_account} успешно изменена.")

                        elif option == 'ip':
                            cursor.execute("UPDATE settings SET twofa_active = ? WHERE nickname = ?", (new_value, selected_account))
                            conn.commit()
                            send_vk_message(user_id, f"⚙️ | Опция 'IP' для аккаунта {selected_account} успешно изменена.")

                        elif option == 'cid':
                            if value == 'вкл':
                                send_vk_message(user_id, f"🚫 | Опция 'CID' не может быть включена в боте.\n💡 | что бы включить защиту по CIDу зайдите на сервер под ником  {selected_account} и напишите /2fa cid-on.")
                            elif value == 'выкл':
                                cursor_protect.execute("DELETE FROM cid WHERE player = ?", (selected_account,))
                                conn_protect.commit()
                                send_vk_message(user_id, f"⚙️ | Опция 'CID' для аккаунта {selected_account} успешно отключена.")
                            else:
                                send_vk_message(user_id, "🚫 | Неверный формат. Используйте '/настройки cid выкл'.")

                        elif option == 'skin':
                            if value == 'вкл':
                                send_vk_message(user_id, f"🚫 | Опция 'SKIN' не может быть включена в боте.\n💡 | чтобы включить защиту по SKINу зайдите на сервер под ником {selected_account} и напишите /2fa skin-on.")
                            elif value == 'выкл':
                                cursor_protect.execute("DELETE FROM skin WHERE player = ?", (selected_account,))
                                conn_protect.commit()
                                send_vk_message(user_id, f"⚙️ | Опция 'SKIN' для аккаунта {selected_account} успешно отключена.")
                            else:
                                send_vk_message(user_id, "🚫 | Неверный формат. Используйте '/настройки skin выкл'.")

                        else:
                            send_vk_message(user_id, "🚫 | Неверный формат. Используйте '/настройки [опция] [вкл/выкл]' или '/настройки'.")
                    ...
                    # Вывод текущих настроек, если команда без аргументов
                        ...
                        if settings is None:
                            ...
                            cid_status = "⛔"
                            skin_status = "⛔"
                            send_vk_message(user_id, f"⚙️ | Для аккаунта {selected_account} были созданы настройки по умолчанию.\n\n📩 | Рассылка: {mailing_status}\n⛓️ | 2FA: {fa_status}\n🖥 | CID: {cid_status}\n👤 | SKIN: {skin_status}")
                        else:
                    ...
                    try:
                        ...
                        if users:
                            ...
                            send_vk_message(user_id, f"🚫 | Нет данных для страницы {page}.")
                    except ValueError:
                        send_vk_message(user_id, "🚫 | Неверный формат. Используйте /юзер-лист [номер страницы].")
  #              else:
  #                  send_vk_message(user_id, "🚫 | Команда не распознана. Используйте "помощь" для списка команд.")
